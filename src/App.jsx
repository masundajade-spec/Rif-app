...
const AI_GLOBAL_CAP = 4.50; // stop all generation once total spend hits this
const AI_USER_DAILY_LIMIT = 8; // max fresh generations per user per day
const AI_ESTIMATED_COST = 0.03; // rough cost per generation for tracking

async function checkAiUsageAllowed(userId) {
  // Check global total spend
  const { data: allUsage } = await supabase
    .from("ai_usage_log")
    .select("estimated_cost");
  const totalSpent = allUsage ? allUsage.reduce((sum, u) => sum + (u.estimated_cost || 0), 0) : 0;

  if (totalSpent >= AI_GLOBAL_CAP) {
    return { allowed: false, reason: "global" };
  }

  // Check this user's usage today
  ...
  if (userUsage && userUsage.length >= AI_USER_DAILY_LIMIT) {
...
function NotesView({ t, user, isMobile }) {
  ...
  async function generateNotes(topicName, forceRegenerate = false) {
    ...
    try {
      ...
      // Not cached - check usage limits before calling AI
      const usageCheck = await checkAiUsageAllowed(user.id);
      if (!usageCheck.allowed) {
        setNotes(
          usageCheck.reason === "global"
            ? "🚫 RIF-App has reached its AI credit limit for now. New topics can't be generated at the moment - please check back soon! Previously generated topics still work normally."
            : "🚫 You've reached your daily limit of AI-generated notes. Try again tomorrow, or explore topics that have already been generated by other students!"
        );
        setLoading(false);
        return;
      }
      ...
      if (data.content && data.content[0]) {
        ...
        await supabase.from("notes_cache").upsert({
          ...
          generated_by: user.id,
                ...
                await logAiUsage(user.id, "notes");
      } else {
    ...
    <div style={{ minHeight: "100vh", background: "#0A1628", padding: "40px 32px" }}>
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        ...
        {[
          ...
          ["6. Data Storage on Your Device", "RIF-App uses your browser's local storage (not traditional cookies) to keep you logged in and remember your preferences. We do not use advertising cookies, tracking pixels, or track you across other websites."],
    ...
    try {
      ...
      if (cached && cached.cards) {
        ...
        if (!usageCheck.allowed) {
          alert(
            usageCheck.reason === "global"
              ? "RIF-App has reached its AI credit limit for now. New topics can't be generated at the moment - please try an existing subject instead!"
              : "You've reached your daily limit of AI-generated flashcards. Try again tomorrow!"
          );
...
- Keep it relevant to the Zimbabwe ${filter} curriculum`,
        ...
        await supabase.from("flashcards_ai_cache").upsert({
          ...
          generated_by: user.id,
                ...
                await logAiUsage(user.id, "flashcards");
      }
